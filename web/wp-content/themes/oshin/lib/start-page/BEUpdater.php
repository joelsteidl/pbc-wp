...
class BEUpdater {
	...
	public function settings_field() {
		...
		add_settings_field( 'token', esc_html__( 'Token', 'oshin' ), array( $this, 'render_token_field' ), self::$option_group_slug );
	}
			...
			if ( empty( $oshin_purchase_data ) || ! be_is_theme_valid() ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( 200 == wp_remote_retrieve_response_code( $response ) ) {
						...
						$body = wp_remote_retrieve_body( $response );
						$response_data = json_decode( $body );
						$theme_message = esc_html__( 'Something went wrong. Please try again later.', 'oshin' );
