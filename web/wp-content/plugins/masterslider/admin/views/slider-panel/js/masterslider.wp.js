...
;(function(){
	...
	MSPanel.Settings = Ember.Model.extend({
        ...
        responsiveSize   : attr('boolean', {defaultValue: false}),
        tabletWidth      : attr('number', {defaultValue: defaults.tabletWidth || 768}),
        tabletHeight     : attr('number'),
        phoneWidth       : attr('number', {defaultValue: defaults.phoneWidth || 480}),
        phoneHeight      : attr('number'),
	...
	MSPanel.Layer = Ember.Model.extend({
        ...
        tabletOffsetY     : attr('number'),
        tabletOffsetX     : attr('number'),
        tabletOrigin      : attr('string'),
        phoneOffsetX      : attr('number'),
        phoneOffsetY      : attr('number'),
        phoneOrigin       : attr('string'),
        hideOn            : attr('string'),
...
MSPanel.PositionOrigin = Ember.View.extend({
    ...
    didInsertElement: function(){
        ...
        this.$('.msp-origin-btn').click(function(){
            const layer = that.get('layer');
            const bp = that.get('controller.parentController.activeBreakpoint');
            const origin = MSPanel.getLayerResponsiveOriginKey(bp);
    ...
    onValueChanged: function(){
        ...
        const bp = this.get('controller.parentController.activeBreakpoint');
        ...
        var value = MSPanel.getResponsiveValue(layer, MSPanel.getLayerResponsiveOriginKey, bp),
            newNode = this.$('.msp-origin-' + value).addClass('msp-origin-btn-selected');

        this.set('selectedNode', newNode);

    }.observes('layer', 'layer.origin', 'layer.tabletOrigin','layer.phoneOrigin', 'controller.parentController.activeBreakpoint')
...
MSPanel.ResponsiveSwitch = Ember.View.extend({
    classNames: ['msp-responsive-control'],
    layer: null,
    selectedNode: null,
    template: Ember.Handlebars.compile(
        '<button data-value="desktop" class="msp-responsive-btn msp-responsive-btn-desktop"><span class="msp-ico msp-ico-desktop"></span></button>'+
        '<button data-value="tablet" class="msp-responsive-btn msp-responsive-btn-tablet"><span class="msp-ico msp-ico-tablet"></span></button>'+
        '<button data-value="phone" class="msp-responsive-btn msp-responsive-btn-phone"><span class="msp-ico msp-ico-phone"></span></button>'),

    didInsertElement: function(){
        var that = this;
        this.$('.msp-responsive-btn').click(function(e){
            var bp = 'desktop';
            if ( e.currentTarget.dataset && e.currentTarget.dataset.value ) {
                bp = e.currentTarget.dataset.value
            }
            that.get('controller.parentController').send('changeActiveBreakpoint', bp);
        });
        this.onBreakpointChange();
    },

    onBreakpointChange: function () {
        var bp = this.get('controller.parentController.activeBreakpoint');
        this.$('.msp-responsive-btn').removeClass('msp-btn-selected');
        this.$('.msp-responsive-btn-' + bp).addClass('msp-btn-selected');
    }.observes('controller.parentController.activeBreakpoint')
});
...
MSPanel.BreakpointSelect = Ember.View.extend({
    classNames: ['msp-responsive-control'],
    layer: null,
    template: Ember.Handlebars.compile(
        '<button data-value="desktop" class="msp-responsive-btn msp-responsive-btn-desktop"><span class="msp-ico msp-ico-desktop"></span></button>'+
    ...
    didInsertElement: function(){
        var that = this;
        var btns = this.$('.msp-responsive-btn');
        btns.click(function(e){
            var $this = that.$(e.currentTarget);
            $this.toggleClass('msp-btn-selected');

            var hideOn = [];
            that.$('.msp-btn-selected').each( function( index, btn ) {
                hideOn.push(that.$(btn).data('value'));
            } );

            that.set('layer.hideOn', hideOn.join(','));
        });
        this.onBreakpointChange();
    },

    onBreakpointChange: function () {
        var hideOn = (this.get('layer.hideOn') || '').split(',');
        var that = this;
        this.$('.msp-responsive-btn').removeClass('msp-btn-selected');
        hideOn.forEach(function( hideBp ) {
            that.$('.msp-responsive-btn-' + hideBp).addClass('msp-btn-selected');
        });
    }.observes('layer', 'layer.hideOn')
});



/* ------------------------------------------------------- *\
                    Responsive Position Input
\* --------------------------------------------------------*/
MSPanel.ResponsivePositionInput = Ember.View.extend({
    classNames: ['msp-origin-control'],
    axis: null,
    layer: null,
    value: null,
    selectedNode: null,
    template: Ember.Handlebars.compile('{{number-input value=view.value min=null}}'),
    didInsertElement: function(){
        this.onValueChanged();
    },
    ...
    onValueChanged: function(){
        ...
        const layer = this.get('layer');
        const that = this;
        const value = MSPanel.getResponsiveValue(layer, function(bp) {
            return  MSPanel.getResponsiveKey(bp, 'offset' + that.get('axis').toUpperCase())
        }, bp);
        ...
        'layer.tabletOffsetY',
        'layer.phoneOffsetY',
        'controller.parentController.activeBreakpoint'
    ),
    ...
    onChange: function() {
        ...
        const layer = this.get('layer');
        if ( layer ) {
            layer.set(MSPanel.getResponsiveKey(bp, 'offset' + this.get('axis').toUpperCase()), this.get('value'));
        }
    }.observes('value')
});
...
MSPanel.StageArea = Ember.View.extend({
    ...
    template : Ember.Handlebars.compile('<div class="msp-metabox-row"><div class="msp-stage-top-toolbar">'+
                                            '<label>'+(__MSP_LAN.sv_004 || 'Responsive : ')+' </label> {{view MSPanel.ResponsiveSwitch}}'+
										...
										'<div class="msp-stage-container">{{view MSPanel.Stage}}</div>'+
...
MSPanel.Stage = Ember.View.extend({
	...
	resize : function(){
        ...
        const settings = this.get('controller.sliderSettings');

		var w = MSPanel.getResponsiveValue(settings, function() { return MSPanel.getResponsiveKey(bp, 'width') } , bp),
			h = this.get('controller.sliderSettings.height');

        if ( this.get('controller.sliderSettings.responsiveSize') ) {
            h = MSPanel.getResponsiveValue(settings, function() { return MSPanel.getResponsiveKey(bp, 'height') } , bp);
        }
	...
	}.observes('controller.sliderSettings.responsiveSize', 'controller.sliderSettings.width' , 'controller.sliderSettings.height' , 'parentView.zoom', 'controller.parentController.activeBreakpoint').on('didInsertElement'),
...
MSPanel.StageLayer = Ember.View.extend({
	...
	updateLayerPostion: function(x , y){
        this.beginPropertyChanges();
        var bp = this.get('controller.parentController').activeBreakpoint;
        var offsetX = bp === 'desktop' ? 'layer.offsetX' : 'layer.' + bp + 'OffsetX';
        var offsetY = bp === 'desktop' ? 'layer.offsetY' : 'layer.' + bp + 'OffsetY';

		x = x || this.$().position().left;
		y = y || this.$().position().top;

		var origin = MSPanel.getResponsiveValue(this.get('layer'), MSPanel.getLayerResponsiveOriginKey, bp),
		...
		switch ( origin.charAt(0) ) {
			case 't':
				...
				break;
			case 'm':
				this.set(offsetY, y - (this.get('parentView.height') - h) / 2);
				break;
			case 'b':
				this.set(offsetY, this.get('parentView.height') - h - y );
				break;
		}
		...
		switch ( origin.charAt(1) ) {
			case 'l':
				...
				break;
			case 'c':
				this.set(offsetX, x - (this.get('parentView.width') - w) / 2);
				break;
			case 'r':
				this.set(offsetX, this.get('parentView.width') - w - x);
				break;
		}
	...
	repositionLayer: function(){
        ...
        var layer = this.get('layer');
        var bp = this.get('controller.parentController').activeBreakpoint;
        var offset = MSPanel.getResponsiveOffset(bp, layer);
		var origin = MSPanel.getResponsiveValue(layer, MSPanel.getLayerResponsiveOriginKey, bp),
			x = offset.x,
			y = offset.y,
			w = this.get('hasImg') ? Math.max(this.$().outerWidth(), this.$('img').width()) : this.$().outerWidth(),
			h = this.get('hasImg') ? Math.max(this.$().outerHeight(), this.$('img').height()) : this.$().outerHeight();
	...
	}.observes('layer.origin', 'layer.phoneOrigin', 'layer.tabletOrigin'),
...
MSPanel.getLayerResponsiveOffsetKey = function(bp, axis) {
    if (axis === 'x') {
        return bp === 'desktop' ? 'offsetX' : bp + 'OffsetX';
    }

    return bp === 'desktop' ? 'offsetY' : bp + 'OffsetY';
}

MSPanel.getLayerResponsiveOriginKey = function(bp) {
    return bp === 'desktop' ? 'origin' : bp + 'Origin';
}

MSPanel.getResponsiveKey = function(bp, key) {
    return bp === 'desktop'? key : bp + key.charAt(0).toUpperCase() + key.slice(1);
}

MSPanel.getResponsiveValue = function(target, mapper, bp) {
    const bpArr = ['phone', 'tablet', 'desktop'];
    const mapped = mapper(bp);
    const sliceBp = bpArr.slice(bpArr.indexOf(bp));
    var value;
    ...
    sliceBp.every(function(currentKey) {
        const mapped = mapper(currentKey);

        if (!Ember.isEmpty(target.get(mapped))) {
            value = target.get(mapped);
           return false;
        }
        return true;
     });
...
MSPanel.getResponsiveOffset = function(bp, layer) {
    return {
        x: MSPanel.getResponsiveValue(layer, function(bp) { return MSPanel.getLayerResponsiveOffsetKey(bp, 'x') }, bp),
        y: MSPanel.getResponsiveValue(layer, function(bp) { return MSPanel.getLayerResponsiveOffsetKey(bp, 'y') }, bp)
    }
}

MSPanel.getResponsiveOrigin = function(bg, layer) {
    return MSPanel.getResponsiveValue(layer, MSPanel.getLayerResponsiveOriginKey, bp);
}
;
...
MSPanel.SlidesController = Ember.ArrayController.extend({
    ...
    actions: {
        ...
        changeActiveBreakpoint: function(breakpoint) {
            if(breakpoint === this.get('activeBreakpoint')) return;
            this.set('activeBreakpoint', breakpoint);
        }
    }
});
